...
|---|---|
| §11 · Estados de identidad: pendiente, en revisión, verificada, rechazada, vencida, suspendida | Son los estados canónicos (`ESTADOS-Y-EVENTOS.md`). Genesis ID ya tiene `vencida` y publica los seis con `estadoPublicado` (`genesis-id/src/motor/estados.ts`): iniciada, datos, documento y biometria salen como «pendiente» |
| §11 · **Un solo puente** entre Genesis ID y Veta Wallet | **Unificado el 26-sep (plan v0.3, tarea 0.6).** El canónico es `infra/veta-wallet-backend/lib/genesisPuente.js`, copiado byte a byte en `infra/mytokenpay-api/src/lib/genesisPuente.js`; `infra/genesis-proxy/` queda retirado (solo reexporta). `probar-puente-genesis.mjs` falla si las copias divergen |
| §11 · Cada vínculo registra la dirección de billetera | Condición del **límite de exposición por identidad** (SFSP-200 §0): sin la dirección en el vínculo, la exposición agregada no se puede calcular y la compra en Mercado de Crecimiento devuelve `BLOCKED_DECISION`. **Desde el 26-sep** el puente, `POST /api/v1/vinculos` de Genesis ID y el SSO de Ordenex rechazan el vínculo sin dirección válida (`VINCULO_SIN_DIRECCION`, `VINCULO_DIRECCION_INVALIDA`); los vínculos viejos sin ella se cuentan con `genesis-id/src/migraciones/v03-estados-y-vinculos.ts` |
