/* COPIA DEL MOTOR DE VELAS DE ORDENEX (apps-web/ordenex/velas.js).
 *
 * La billetera y la casa de cambio pintan las velas IGUAL a proposito: son la
 * misma casa. Si se arregla algo aqui o alla, copiar el archivo entero es la
 * forma de sincronizar — con una excepcion: el PELLIZCO de dos dedos se añadio
 * primero en esta copia (buscar «pellizco» en gestos()) y conviene llevarlo a
 * Ordenex en la proxima pasada.
 */
...
const VELAS = (() => {
  ...
  function gestos(canvas, { total, alCambiar } = {}) {
    ...
    /* El pellizco (añadido de la billetera). Dos punteros vivos: su distancia
       manda el factor y su punto medio es el ancla, con el mismo acercar()
       que usa la rueda — una sola matematica de zoom, dos gestos que la
       llaman. */
    const punteros = new Map();
    let pinza = null;        // { d0, ventana0 }
    ...
    const alAgarrar = e => {
      ...
      const b = canvas.getBoundingClientRect();
      if (e.pointerId != null) {
        punteros.set(e.pointerId, e.clientX - b.left);
        if (punteros.size === 2) {
          // Empieza el pellizco: el arrastre muere y se anota la ventana base.
          const [x1, x2] = [...punteros.values()];
          const t = cuantas();
          pinza = { d0: Math.max(14, Math.abs(x1 - x2)),
                    ventana0: ventana ? { ...ventana } : { desde: 0, hasta: t } };
          arrastre = null;
          e.preventDefault();
          return;
        }
    ...
    const alArrastrar = e => {
      if (e.pointerId != null && punteros.has(e.pointerId)) {
        const b2 = canvas.getBoundingClientRect();
        punteros.set(e.pointerId, e.clientX - b2.left);
      }
      if (pinza && punteros.size === 2) {
        /* La misma matematica de acercar(), pero continua: la ventana BASE se
           estira por la razon de distancias, anclada en el punto medio. Contra
           la base y no contra la ventana viva, para que el gesto no se
           acumule sobre si mismo y se dispare. */
        const [x1, x2] = [...punteros.values()];
        const d = Math.max(14, Math.abs(x1 - x2));
        const medio = (x1 + x2) / 2;
        const t = cuantas(); if (!t) return;
        const base = pinza.ventana0;
        const largo0 = Math.max(1, base.hasta - base.desde);
        const largo = Math.max(MIN_VELAS, Math.min(t, largo0 * (pinza.d0 / d)));
        const ancho = canvas.clientWidth || 1;
        const fAncla = Math.min(1, Math.max(0, medio / ancho));
        const iAncla = base.desde + fAncla * largo0;
        let desde = Math.round(iAncla - fAncla * largo);
        let hasta = Math.round(desde + largo);
        if (desde < 0) { hasta -= desde; desde = 0; }
        if (hasta > t) { desde -= (hasta - t); hasta = t; }
        ventana = (desde <= 0 && hasta >= t) ? null : { desde: Math.max(0, desde), hasta };
        avisar();
        e.preventDefault();
        return;
      }
    ...
    const alSoltar = e => {
      if (e && e.pointerId != null) punteros.delete(e.pointerId);
      if (punteros.size < 2) pinza = null;
      arrastre = null; canvas.style.cursor = 'crosshair';
    };
    ...
    /* 'none' y no 'pan-y': el pellizco de dos dedos es nuestro, y con pan-y el
       navegador se lo quedaria para hacer zoom de pagina. El costo es que el
       scroll vertical no arranca DESDE la grafica, que en una ficha con la
       grafica a media pantalla es un costo chico. */
