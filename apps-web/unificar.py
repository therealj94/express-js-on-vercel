...
# --- el icono, dentro del HTML -------------------------------------------
ruta_icono = os.path.join(ORIGEN, 'assets', 'veta-icon.png')
crudo = open(ruta_icono, 'rb').read()
try:
    ...
    pass
icono = 'data:image/png;base64,' + base64.b64encode(crudo).decode()
html = html.replace('assets/veta-icon.png', icono)
...
codigo = '\n'.join(leer(n) for n in ('qr.js', 'i18n.js', 'app.js'))
assert '</script' not in codigo, 'un script cierra la etiqueta y romperia el HTML'
bloque = ('<script>\nif (!window.__legal) {\n' + codigo +
          '\nwindow.VETA = VETA;\n}\n</script>')
html = re.sub(r'<script src="qr\.js"></script>\s*'
              r'<script src="i18n\.js"></script>\s*'
              r'<script src="app\.js"></script>', lambda _: bloque, html, count=1)
