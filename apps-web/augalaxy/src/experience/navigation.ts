...
interface ExperienceData {
 ...
 help:boolean;ready:boolean;unsupported:boolean;fps:number;hovered:string|null;
...
export const useExperience=create<ExperienceState>((set)=>({stage:'gate',selected:null,windowId:null,settings:false,directory:false,help:false,ready:false,unsupported:false,fps:60,hovered:null,journey:null,rendererChoice:null,rendererActual:'lite',webglAvailable:null,set}));
...
export const navigation={
 ...
 orbit(dx:number,dy:number){if(useExperience.getState().stage==='transit')return;this.lastInteraction=now();this.yaw+=dx*.0019;this.pitch=Math.max(-.35,Math.min(1.18,this.pitch+dy*.0014));sound.motion(Math.hypot(dx,dy),dx);},
 ...
 galaxies(){this.zoom=1;this.yaw=0;this.pitch=.38;this.lastInteraction=now();useExperience.getState().set({stage:'galaxies',selected:null,journey:null,windowId:null});sound.travel('focus');},
 home(){this.zoom=1;this.yaw=0;this.pitch=.6;this.lastInteraction=now();this.hovered=null;useExperience.getState().set({selected:null,stage:'system',windowId:null,journey:null,hovered:null});},
