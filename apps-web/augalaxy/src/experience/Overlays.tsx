...
export function Settings({onClose}:{onClose:()=>void}){
 ...
 const renderState=useExperience(),fps=renderState.fps,currentMode=renderState.rendererChoice||prefs.defaultRenderer;
 ...
 {tab==='visual'&&<><p className="eyebrow">{es?'TU UNIVERSO, A TU RITMO':'YOUR UNIVERSE, YOUR PACE'}</p><h2>{es?'Elige tu experiencia.':'Choose your experience.'}</h2>
 <div className="renderer-options" role="group" aria-label={es?'Modo de experiencia':'Experience mode'}><button className={currentMode==='pro'?'selected':''} aria-pressed={currentMode==='pro'} onClick={()=>renderState.set({rendererChoice:'pro'})}><span className="mode-heading"><strong>PRO</strong><span>WebGL</span></span><b>{es?'Experiencia completa':'Full experience'}</b><small>{es?'Volumen 3D, atmósferas, corona solar y luz en tiempo real.':'3D volume, atmospheres, solar corona and real-time light.'}</small></button><button className={currentMode==='lite'?'selected':''} aria-pressed={currentMode==='lite'} onClick={()=>renderState.set({rendererChoice:'lite'})}><span className="mode-heading"><strong>LITE</strong><span>{es?'Ligero':'Lightweight'}</span></span><b>{es?'Más compatibilidad':'Wider compatibility'}</b><small>{es?'La misma navegación, con menor carga gráfica.':'The same navigation, with a lighter graphics workload.'}</small></button></div>
 <div className="renderer-status" role="status">{es?'Modo activo: ':'Active mode: '}<strong>{renderState.rendererActual.toUpperCase()}</strong>{currentMode==='pro'&&renderState.webglAvailable===false&&<span>{es?'WebGL no está disponible aquí. Lite mantiene tu experiencia.':'WebGL is unavailable here. Lite keeps your experience running.'}</span>}</div>
 <label className="setting-row"><span><strong>{es?'Modo al iniciar':'Startup mode'}</strong><small>{es?'Se guarda en este dispositivo.':'Saved on this device.'}</small></span><select value={prefs.defaultRenderer} onChange={e=>change('defaultRenderer',e.target.value as 'pro'|'lite')}><option value="pro">Pro · WebGL</option><option value="lite">Lite</option></select></label>
 ...
 <Toggle label={es?'Órbitas automáticas':'Automatic orbits'} detail={es?'Se detienen al señalar o seleccionar una app.':'Pause when you point to or select an app.'} value={prefs.autoOrbit} onChange={v=>change('autoOrbit',v)}/>
 <p className="quiet-note">{es?'Los nombres de las apps permanecen visibles durante la navegación. Las líneas indican a qué planeta pertenece cada nombre.':'App names remain visible while navigating. Lines identify the planet belonging to each name.'}</p>
 <Slider label={es?'Sensibilidad del giro':'Orbit sensitivity'} detail={es?'Arrastra el espacio en la dirección que quieres moverlo.':'Drag space in the direction you want it to move.'} value={prefs.orbitSensitivity} min={.65} max={1.4} step={.05} format={prefs.orbitSensitivity.toFixed(2)+'×'} onChange={v=>change('orbitSensitivity',v)}/>
 ...
 <button className="text-button" onClick={()=>{onClose();navigation.home();useExperience.getState().set({tutorial:true});}}><Icon name="help"/>{es?'Ver tutorial del sistema':'View system tutorial'}</button><div className="shortcut-list"><span>Tab</span><p>{es?'Recorrer controles y apps':'Move through controls and apps'}</p><span>Enter</span><p>{es?'Activar selección':'Activate selection'}</p><span>Esc</span><p>{es?'Cerrar ventana o volver al sistema':'Close window or return to system'}</p><span>+ / −</span><p>{es?'Acercar / alejar':'Zoom in / out'}</p><span>← / →</span><p>{es?'Elegir planeta anterior o siguiente':'Choose the previous or next planet'}</p><span>Shift + ← / →</span><p>{es?'Girar el universo':'Orbit the universe'}</p></div></>}
 ...
 <dl className="system-facts"><div><dt>{es?'Entorno':'Environment'}</dt><dd>{es?'Vista privada de diseño':'Private design preview'}</dd></div><div><dt>{es?'Cuentas y fondos':'Accounts & funds'}</dt><dd>{es?'Sin conexión':'Not connected'}</dd></div><div><dt>{es?'Motor visual':'Visual renderer'}</dt><dd>{renderState.rendererActual==='pro'?'Pro · WebGL':'Lite · Canvas'}</dd></div><div><dt>{es?'Rendimiento observado':'Observed performance'}</dt><dd>{fps} FPS</dd></div><div><dt>{es?'Preferencias':'Preferences'}</dt><dd>{storageAvailable?(es?'Guardadas en este navegador':'Saved in this browser'):(es?'Solo para esta sesión':'This session only')}</dd></div></dl>
 ...
 <p className="quiet-note">{es?'Mapa estelar: ':'Star map: '}<a href="https://svs.gsfc.nasa.gov/3895" target="_blank" rel="noreferrer">NASA / Goddard Space Flight Center Scientific Visualization Studio</a>.</p>
 <button className="text-button" onClick={()=>{reset();renderState.set({rendererChoice:null});airtouch.stop();}}>{es?'Restaurar preferencias':'Reset preferences'}</button></>}
 </section></div></Modal>;
}
