...
export default function Experience({embedded=false}:{embedded?:boolean}){
 ...
 /* ══ DOS VIDAS DEL MISMO MOTOR ═══════════════════════════════════════════
    SOLO: es un sistema operativo entero —cabecera, dock, fichas de cada
    mundo— y se basta.
    DENTRO DE LA WALLET: es LA ESCENA Y NADA MÁS. La casa ya tiene su marca,
    su menú, su entrada y sus apps; dibujar los nuestros encima no es sumar,
    es duplicar: dos «ORDEN GLOBAL» pisados en la misma esquina, dos filas de
    apps que hacen cosas distintas, y una intro nuestra tapando la suya. Así
    salió a producción y así se vio. Aquí adentro mandan la casa y el cielo. */
 const soloEscena=embedded;
 ...
 {!state.immersive&&!intro&&!travelling&&!cinema&&!soloEscena&&<header className="os-header"><button className="brand-button" aria-label={es?'Inicio Orden Global':'Orden Global home'} onClick={()=>!isGate&&navigation.home()}><Brand/></button>
 ...
 {!(soloEscena&&isGate)&&<PlanetLabels/>}
 {!soloEscena&&<>
 ...
 {/* LA PISTA QUE FALTABA. Al dejar de dibujar nuestra cáscara dentro de la
     wallet se fue con ella la única línea que decía qué hacer: quedaba un
