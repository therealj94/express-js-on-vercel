...
(function () {
  ...
  function medir() {
    ...
    /* Antes se pintaban aquí las 56 esferas (siete casas por ocho ángulos
       de luz) de una vez, y eso era casi medio segundo de hilo principal al
       cargar en un teléfono. Ahora cada una se pinta la PRIMERA vez que un
       cuadro la pide, que es cuando el planeta pasa por ese ángulo: en el
       primer cuadro son siete, y el resto van llegando repartidas. */
    bolas = { _lado: lado };
    CASAS.forEach(function (casa) { bolas[casa.id] = new Array(SECTORES); });
  ...
  function cuadro(ahora) {
    ...
    function estampar(p) {
      ...
      var sector = ((Math.round(ax / (Math.PI * 2) * SECTORES) % SECTORES) + SECTORES) % SECTORES;
      var b = lote[sector];
      if (!b) {
        var a = (sector / SECTORES) * Math.PI * 2;
        b = lote[sector] = pintarBola(p.casa, bolas._lado, Math.cos(a), Math.sin(a));
      }
      var l = b.width * p.esc;
    ...
    if (!enLista()) puestos.forEach(function (p) {
      ...
      var radio = (bolas._lado || 0) * p.esc * 0.34;
