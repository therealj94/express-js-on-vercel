"""Música suave, efectos y mezcla con la voz de Zabra (ElevenLabs v3). La voz manda. Todo cae en las palabras de tiempos.json.
...
for n_, a, b, f in [(0, F(6).ini, F(9).ini - .5, A2 / 2), (1, F(9).ini, F(11).ini, G2 / 2), (2, F(11).ini, F(12).ini - .2, A2 / 2)]:
    x = a
    while x < b:
        put(M, pulso(f * 2, .35, 9), x, .05 * (1 + .5 * (n_ == 2)))
        x += paso
...
put(X, silbido(1.2, 400, 7000, .10), F(13).fin + .1)                   # firma Orden Global
for k in range(6): put(X, campana(1760 * (1.12 ** k), 1.2), F(13).fin + .55 + k * .17, .025, -.5 + .2 * k)
put(X, impacto(3.0, 55, 27.5, .1), F(13).fin + .2, .5)
...
# ---- ducking: la música baja cuando habla Zabra ----
ve = lp(np.abs(v), 3); ve = ve / (ve.max() + 1e-9)
duck = 1 - .7 * np.clip(lp(np.clip(ve * 6, 0, 1), 2), 0, 1)
Vn = V / (np.abs(V).max() + 1e-9) * .95
Mn = M / (np.abs(M).max() + 1e-9) * .32 * duck
Xn = X / (np.abs(X).max() + 1e-9) * .45 * (1 - .45 * (duck < .8))
mix = Vn + Mn + Xn
if len(sys.argv) > 2:  # medición: voz contra fondo mientras habla
    hab = lp(np.abs(v), 5) > .01
    db = lambda x: 20 * np.log10(np.sqrt(np.mean(x ** 2)) + 1e-12)
    print('voz sobre fondo mientras habla: %.1f dB' % (db(Vn[0][hab]) - db((Mn + Xn)[0][hab])))
mix = hp(mix, 25)
fade = env(0, DUR, .05, 1.2); mix *= fade
pk = np.abs(mix).max()
mix = np.tanh(mix / pk * 1.1) / np.tanh(1.1) * .9
