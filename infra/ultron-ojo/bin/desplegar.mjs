/* Despliega EL OJO DE ULTRON a Heroku por la API de la plataforma, igual que Ordenex.
 *
 *   node infra/ultron-ojo/bin/desplegar.mjs [--probar-solo] [--crear]
 *
 ...
 * (`git archive`), no lo que hay en el disco.
 *
...
const PREFIJO = 'infra/ultron-ojo';
const APP = process.env.HEROKU_APP || 'ultron-ojo';
...
let urlApp = process.env.OJO_URL || '';
...
if (sinCommitear) {
  rojo('Hay cambios en infra/ultron-ojo sin commitear. El paquete va del commit, no del disco:');
...
const p = salud;
const fila = (k, bien, que) => console.log(`  ${bien ? '✓' : '·'} ${k.padEnd(11)} ${que}`);
fila('servicio', p.servicio === 'ultron-ojo', p.servicio || '?');
fila('clave', p.conClave, p.conClave ? 'puesta' : 'SIN PONER — el ojo no atiende a nadie (OJO_CLAVE)');
fila('navegador', true, p.navegador ? 'ya encendido' : 'se enciende con la primera página');
fila('memoria', p.memoriaMb < 400, `${p.memoriaMb} MB de los 512 del dyno`);
