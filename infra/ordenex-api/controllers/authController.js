// La puerta de Ordenex: el SSO de Genesis y su refresco.
//
...
// paso (su POST /genesis/sso/token), la web de Ordenex lo trae a POST
...
// casa: JWT HS256 firmado con ORDENEX_TOKEN — nunca el PASS_TOKEN de la
// wallet, porque compartir el secreto de firma seria compartir todas las
// sesiones, y un compromiso de Ordenex no puede volverse uno de la billetera.
//
...
async function sso(req, res) {
  ...
  if (!secreto) {
    ...
    console.error('[auth] ORDENEX_TOKEN no esta puesto: no se puede emitir ninguna sesion');
  ...
  if (!perfil || typeof perfil !== 'object') {
    console.error('[auth] Genesis verifico el token pero no mando perfil: a la clave de ordenex le falta el alcance gid.perfil');
...
async function refresh(req, res) {
  const secreto = process.env.ORDENEX_TOKEN;
  if (!secreto) {
    console.error('[auth] ORDENEX_TOKEN no esta puesto: no se puede canjear ningun refresh');
    return res.status(503).json({ error: 'El servicio no esta configurado.', codigo: 'SIN_CONFIGURAR' });
  }
