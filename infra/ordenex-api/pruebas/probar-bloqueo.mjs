/* El bloqueo del ecosistema, del lado de Ordenex.
 *
 *   node pruebas/probar-bloqueo.mjs
 *
 * LO QUE ESTO TAPA. Genesis ID puede bloquear a alguien en todo el ecosistema,
 * pero Ordenex no vive de ese pase: lo usa UNA vez, en /auth/sso, y después
...
decir('bloqueado en Genesis, fuera de Ordenex');
{
...
decir('la memoria corta');
{
  ...
  // Ordenex se caería cada vez que Genesis tosa y Genesis recibiría el
  // tráfico de Ordenex entero.
...
decir('si Genesis no contesta');
{
  ...
  comprobar(pocoDespues.puede === true,
    'un tropiezo corto de Genesis NO tumba Ordenex: se usa lo último que dijo');
