/* Despliega el BACKEND DE VETA WALLET a Heroku por la API de la plataforma, igual que Ordenex.
 *
 *   node infra/veta-wallet-backend/bin/desplegar.mjs [--probar-solo] [--crear]
 *
...
const PREFIJO = 'infra/veta-wallet-backend';
const APP = process.env.HEROKU_APP || 'vetawallet';
...
let urlApp = process.env.WALLET_URL || '';
...
if (sinCommitear) {
  rojo('Hay cambios en infra/veta-wallet-backend sin commitear. El paquete va del commit, no del disco:');
...
fila('estado', p.ok !== false, JSON.stringify(p).slice(0, 160));
