#!/bin/bash
set -e
G=$(python3 -c "import json;print(' '.join(str(int(t*1000)) for t,_ in json.load(open('guion_b.json'))))")
ENT=""; FIL=""; MIX=""; i=0
for ms in $G; do
  i=$((i+1)); ENT="$ENT -i vb_${i}.wav"
...
FIL="${FIL}[$k:a]adelay=42500|42500[ck];"; MIX="${MIX}[ck]"
k=$((k+1)); ENT="$ENT -i amb_b.wav"
FIL="${FIL}[$k:a]volume=0.32[amb];"; MIX="${MIX}[amb]"
ffmpeg -v error -y -i corte_B.mp4 $ENT -filter_complex \
  ...
  -map 0:v -map "[a]" -c:v copy -c:a aac -b:a 192k -t 64.33 anuncio_B.mp4
echo "listo: anuncio_B.mp4"
