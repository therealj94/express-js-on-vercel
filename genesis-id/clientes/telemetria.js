...
//
// ⚠ EN UNA APP MÓVIL, LA CLAVE NO VA DENTRO DEL APK. Un APK se descomprime en
// diez segundos. Poné la clave en TU backend y que la app reporte contra un
// endpoint tuyo que reenvíe. Es la misma regla que ya sigue el puente de
// identidad de Veta Wallet.
// ─────────────────────────────────────────────────────────────────────────────
...
async function vaciar() {
  ...
  try {
    const r = await fetch(CONF.url.replace(/\/$/, '') + '/api/v1/telemetria/eventos', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-API-Key': CONF.clave },
