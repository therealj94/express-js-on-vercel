...
El ecosistema son cinco piezas —**Veta Wallet**, **MyTokenPay**, **OrdenExchange**,
...
> MyTokenPay, ordenscan (el explorador) y **OrdenExchange** (la casa de cambio
> P2P, en `ordenexchange/`: entra por la misma puerta que las demás, con clave
> `ordenexchange` y alcances propios; la clave se imprime al redesplegar el
> motor). **AuCorp todavía no está conectado** — no hay código suyo en este
> repositorio.
