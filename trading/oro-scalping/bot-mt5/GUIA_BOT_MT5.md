...
> **v2.40 ULTIMATE — auditoría final (14 agentes revisores + verificación adversarial):** TP1 ya no se marca como hecho si el cierre parcial falla (se reintenta al tick siguiente) y el break-even se reasegura solo; si un reinicio borra las variables del terminal, el TP1 se **reconstruye desde el SL de la propia posición** (la operación nunca queda huérfana); el contador de operaciones cuenta por posición aunque el broker llene la entrada en varios fills. Nuevo: **notificaciones push al móvil** (app MetaTrader, activando tu MetaQuotes ID) y ventana de alerta en el terminal para entradas, TP1, salida inteligente, objetivo/límite del día y cierres; el estado del gráfico muestra el P&L del día en vivo.
>
> **v2.30 — objetivos del día:** las operaciones por día son 100% tuyas (la agresividad ya no las modifica) y ahora el día se detiene solo al cumplirse cualquiera de estos: **objetivo de ganancia** (en % o en $) o **límite de pérdida** (en % o en $). Al detenerse cierra también la posición abierta (configurable) para asegurar el resultado, y el candado es persistente: ni un reinicio del VPS ni un retroceso del equity reabren el día — mañana se libera solo. Ejemplo típico: objetivo $200, límite $100 → el bot corre hasta cobrar 200, perder 100, o agotar sus 4 trades; lo que ocurra primero.
>
> **v2.20 — lote manual y agresividad:** nuevo grupo "🎚 Lote y agresividad". **Lote fijo manual:** pones tu monto de lotes (ej. 0.10) y el bot ejecuta siempre con ese tamaño — con un tope de seguridad: si con el SL de esa señal el lote fijo arriesgara más del 5% del equity (configurable), lo recorta y te lo avisa en el log. **Agresividad:** Normal / Agresivo (umbral −5, +2 ops/día, TP2 ≥ 2.5R, trailing más ceñido, riesgo ×1.5) / Turbo (umbral −8, +4 ops/día, TP2 ≥ 3R, cooldown a la mitad, riesgo ×2). Los frenos de pérdida diaria y de pérdidas máximas siguen activos en todos los modos — la agresividad amplifica el beneficio de las buenas rachas, no elimina la protección.
>
